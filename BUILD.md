...
This guide explains how to deploy SeaPunk Manager to production using **Netlify** for the frontend and **Render** for the backend.
...
SeaPunk Manager uses a **split deployment architecture**:
...
databases:
  - name: seapunk-db # PostgreSQL database
    databaseName: seapunk_manager
    user: seapunk_user
    plan: free
```
...
1. **Connect Repository to Render**
   ...
   - Select the `SeaPunk-Manager` repository
...
2. **Render Auto-Detects Configuration**
   ...
   - Review the services to be created:
     - `seapunk-api` (Web Service)
     - `seapunk-db` (PostgreSQL Database)
...
5. **Verify Deployment**
   ...
   - Visit: `https://seapunk-api.onrender.com/api` (Swagger docs)
   - Check health: `https://seapunk-api.onrender.com/api`
...
2. **Create Web Service**
   ...
   - Configuration:
     ...
     Branch: develop
     Runtime: Node
     Build Command: npm install && npx prisma generate --schema=apps/backend-rest/prisma/schema.prisma && npx prisma migrate deploy --schema=apps/backend-rest/prisma/schema.prisma && npx turbo build --filter=@seapunk/backend-rest
...
buildCommand: |
  ...
  npx turbo build --filter=@seapunk/backend-rest   # Build backend with Turborepo
```
...
[build]
  base = "."
  command = "npm install && turbo build --filter=@seapunk/frontend..."
...
- `--filter=@seapunk/frontend...` - Builds frontend and its dependencies
...
3. **Configure Environment Variables**
   ...
   ```
   NEXT_PUBLIC_API_URL = https://seapunk-api.onrender.com
   ```
...
```bash
turbo build --filter=@seapunk/frontend...
```

The `...` suffix means: **build @seapunk/frontend AND all packages it depends on**.
...
| `DATABASE_URL`   | Auto-generated | From `seapunk-db` database             |
...
- API (Production): https://seapunk-api.onrender.com
- Swagger Docs: https://seapunk-api.onrender.com/api
